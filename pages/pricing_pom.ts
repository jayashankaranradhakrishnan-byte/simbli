...
export class PricingPage {
  ...
  get comparePlansCheckIcons(): Locator {
    return this.page.locator("table.shadcn-matrix-table .check-circle-icon");
  }
  ...
  async pricing_page() {
    ...
    const priceh1 = await validator.getTextCss(this.pricingpageHeader_h1);

    const priceh2 = await validator.getTextCss(this.pricingpageHeader_h2);
    ...
    await expect(this.freePlanCard).toContainText("Access to all 4 agents");
    await expect(this.freePlanCard).toContainText("Limited monthly usage");
    await expect(this.freePlanCard).toContainText(
      "Limited AI content generation",
    );
    await expect(this.freePlanCard).toContainText("Limited social media usage");
    await expect(this.freePlanCard).toContainText(
      "Basic access to Simbli features",
    );
    await expect(this.freePlanCard).toContainText("Community support");
    ...
    await expect(this.basicPlanCard).toContainText("Access to all 4 agents");
    await expect(this.basicPlanCard).toContainText(
    ...
    await expect(this.basicPlanCard).toContainText("Kayal & Tara included");
    await expect(this.basicPlanCard).toContainText("Email support");
    ...
    await expect(this.standardPlanCard).toContainText("Access to all 4 agents");
    await expect(this.standardPlanCard).toContainText(
    ...
    await expect(this.standardPlanCard).toContainText("Kayal & Tara included");
    await expect(this.standardPlanCard).toContainText("Priority email support");
    ...
    await expect(this.proPlanCard).toContainText("Access to all 4 agents");
    await expect(this.proPlanCard).toContainText(
    ...
    await expect(this.proPlanCard).toContainText("Kayal & Tara included");
    await expect(this.proPlanCard).toContainText("Priority support");
    ...
    for (let i = 0; i < (await planCards.count()); i++) {
      ...
      await validator.validateContainerCss(card, expectedCommon);
    }
